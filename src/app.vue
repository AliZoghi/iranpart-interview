  ...
  <main class="min-h-screen bg-slate-950 px-6 py-10 text-slate-100">
    <section class="mx-auto flex max-w-5xl flex-col gap-6">
      <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p class="text-sm font-medium uppercase tracking-wide text-cyan-300">Tickets</p>
      ...
      <p v-if="error" class="rounded-md border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-200">
        {{ error }}
      </p>
        ...
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-300">{{ ticket.description }}</p>
            </div>

            <span class="w-fit rounded-md bg-slate-800 px-3 py-1 text-xs text-slate-300">
              {{ ticket.statusTitle }}
            </span>
          </div>
          ...
          <dl class="mt-4 grid gap-3 text-sm text-slate-400 sm:grid-cols-3">
            ...
            <div>
              ...
              <dd class="mt-1 text-slate-200">{{ ticket.isActive ? 'Yes' : 'No' }}</dd>
            </div>
          </dl>
        </article>
      </div>
