...
export class BaseApi {
  ...
  protected get<T>(url: string, config?: AxiosRequestConfig): Promise<T> {
    return this._request<T>({ ...config, method: 'GET', url: this._resolveUrl(url) })
  }

  protected post<T, TBody = unknown>(url: string, data?: TBody, config?: AxiosRequestConfig): Promise<T> {
    return this._request<T>({ ...config, method: 'POST', url: this._resolveUrl(url), data })
  }

  protected put<T, TBody = unknown>(url: string, data?: TBody, config?: AxiosRequestConfig): Promise<T> {
    return this._request<T>({ ...config, method: 'PUT', url: this._resolveUrl(url), data })
  }

  protected delete<T>(url: string, config?: AxiosRequestConfig): Promise<T> {
    return this._request<T>({ ...config, method: 'DELETE', url: this._resolveUrl(url) })
  }
